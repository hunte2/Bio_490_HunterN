...
```{r}
write.csv(clinic, "/Users/hunter/Desktop/QBIO_490_HunterN/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/hunter/Desktop/QBIO_490_HunterN/analysis_data/brca_clinical_data.csv")
```
...
```{r Exercise 4.1}
x <- clinic$death_days_to
y <- clinic$age_at_diagnosis

plot(x, y, main = "Vital Status vs. Age at Diagnosis", xlab = "Vital Status", ylab = "Age at Diagnosis", pch = 19, xlim = c(0, 6000), ylim = c(0, 100), col = "blue", bg = "red", cex = 1.5)                               
...
```{r Exercise 4.2}
x1 <- c(clinic$er_ihc_score)
y1 <- c(clinic$initial_pathologic_dx_year)

par(mfrow = c(1, 2))  
boxplot(x1, main = "Box Plot of ER IHC Score", ylab = "Value", col = "blue", border = "black", notch = TRUE)
boxplot(y1, main = "Box Plot of ER IHC Score", ylab = "Value", col = "green", border = "black", notch = TRUE)
...
```{r}
cleaned_clinic$death_event <- cleaned_clinic$death_event == "T"
```
...
The p-value is 0.2. It is significantly significant and tells us that age does have a correlation with age and breast cancer survival. 
